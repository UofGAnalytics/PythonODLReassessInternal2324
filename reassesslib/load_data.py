...
def load_data(data_set_id):
    ...
    worker_indices = [
        i for i, ant_type in enumerate(ant_types) if ant_type == "worker"
    ]
    ...
    for i, ant_type in enumerate(ant_types):
        if ant_type == "queen":
            ...
            if i in worker_indices[:num_worker_large]:
                ...
                mandible_sizes[i] = np.random.uniform(
                    2.5, 3.5
                )  # Larger mandibles
            else:
                weights[i] = np.random.uniform(5, 10)
    ...
    # Add outliers to the worker ants
    outlier_index = worker_indices[
        0:2
    ]  # Choose some workers with larger mandibles
    weights[outlier_index] = np.random.uniform(
        15, 20
    )  # Significantly higher weight
    mandible_sizes[outlier_index] = np.random.uniform(
        4.5, 5.5
    )  # Significantly larger mandible size
